...
# إعداد التطوير المحلّي ما لازم يوصل للنشر: سطره بيطلب ملف مو موجود،
# يعني ٤٠٤ بكونسول كل زائر
! grep -q 'config\.local\.js' "$TMP/ok/index.html" \
  && ! grep -rq 'config\.local\.js' "$TMP/ok/kmh123475674/"
check "config.local.js ما وصل للناتج" $?
